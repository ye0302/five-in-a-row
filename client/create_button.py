...
class CreateButton:
    def __init__(self, screen, uc):
        self.screen = screen
        self.uc = uc
        self.own_win_score = None
        self.adverse_win_score = None
        self.list_button = []
        self.game_is_alive = None
        self.own_regret_count = 3
        self.create_circulate_button()
    ...
    def create_circulate_button(self):
    ...
    def create_adverse_scoreboard(self, adverse):
        ...
        :return:
        """
        self.adverse_win_score = Board(self.screen, 100, 40, "win: %s" % adverse.win_count)
        self.adverse_lose_score = Board(self.screen, 100, 80, "lose: %s" % adverse.lose_count)
    ...
    def create_own_scoreboard(self, user):
        ...
        :return:
        """
        self.own_win_score = Board(self.screen, 390, 720, "win: %s" % user.win_count)
        self.own_lose_score = Board(self.screen, 390, 755, "lose: %s" % user.lose_count)
    ...
    def get_regret_button(self):
        self.regret_button = ClickButton(self.screen, 90, 720, "Regret %d" % self.own_regret_count,
                                         (255, 136, 20))

    def get_cannot_click_regret_button(self):
        self.cannot_click_regret_button = ClickButton(self.screen, 90, 720, "Regret", (128, 128, 128))
    ...
    def countdown(self, manager):
        while self.uc.time >= 0 and self.game_is_alive:
            if self.color_button in self.list_button and self.uc.time<=45:
                self.list_button.remove(self.color_button)
            if self.regret_time == 8:
                if self.time_button in self.list_button:
                    self.list_button.remove(self.time_button)  #
                self.get_nowtime_button()
                if self.time_button:
                    self.list_button.append(self.time_button)
                self.uc.time -= 1
            sleep(1)
        if self.uc.can_discard and self.uc.time < 0:
            self.do_over_time(manager)

    def get_nowtime_button(self):
        if self.uc.time <= 45:
            if self.uc.can_discard:
                ...
                self.time_button = Board(self.screen, 240, 740, "countdown: %d" % self.uc.time, (33, 226, 15))
            else:
                ...
                self.time_button = Board(self.screen, 240, 60, "countdown: %d" % self.uc.time, (33, 226, 15))
    ...
    def update_own_score(self, user):
        if self.own_win_score in self.list_button:
            self.list_button.remove(self.own_win_score)
            self.list_button.remove(self.own_lose_score)
        self.create_own_scoreboard(user)
        self.list_button.append(self.own_win_score)
    ...
    def update_adverse_score(self, user):
        if self.adverse_win_score in self.list_button:
            self.list_button.remove(self.adverse_win_score)
            self.list_button.remove(self.adverse_lose_score)
        self.create_adverse_scoreboard(user)
        self.list_button.append(self.adverse_win_score)
        self.list_button.append(self.adverse_lose_score)

    def do_over_time(self, manager):
        self.overtime_button = RemindButton(self.screen, "Over time", 30)
        self.ending_button = RemindButton(self.screen, "You lost", -30)
    ...
    def get_own_regret_button(self):
        self.own_regret_button = RegretBoard(self.screen, "Waiting for agree...", 80)
    ...
    def get_opponent_regret_button(self):
        self.opponent_regret_button = RegretBoard(self.screen, "opponent asks for regret", 200, 50)
        self.list_button.append(self.opponent_regret_button)
        self.agree_button = ClickButton(self.screen, 350, 450, "Agree", (0, 255, 255))
    ...
    def get_disagree_button(self):
        while self.regret_time >= 0:
            if len(self.regret_click_button) == 2:
                ...
                self.list_button.remove(self.disagree_button)
            self.disagree_button = ClickButton(self.screen, 200, 450,
    ...
    def get_quit_button(self):
        self.quit_button = ClickButton(self.screen, 90, 760, "Quit", (128, 128, 128))
